...
var removespc = function (text, exp = "") {
    let checksps = new RegExp('[^a-zA-Z0-9àảãáạăằẳẵắặâầẩẫấậÀẢÃÁẠĂẰẲẴẮẶÂẦẨẪẤẬđĐèẻẽéẹêềểễếệÈẺẼÉẸÊỀỂỄẾỆìỉĩíịÌỈĨÍỊòỏõóọôồổỗốộơờởỡớợÒỎÕÓỌÔỒỔỖỐỘƠỜỞỠỚỢùủũúụưừửữứựÙỦŨÚỤƯỪỬỮỨỰỳỷỹýỵỲỶỸÝỴ' + exp + '\_\\\\/\\(\\)-]', 'g');
    return (text + '').normalize().replace(checksps, '');
}
var jjjg = setTimeout(() => { }, 0);
var setvalthat = (val, that, time = 0) => {
    clearTimeout(jjjg);
    jjjg = setTimeout(() => { that.setState({ 'value': val }) }, time);
}
...
class Mmultiswitch extends React.Component {
    ...
    render() {
        return (
            <div {...this.props} className={"m-ios-switch " + (this.data.class ? this.data.class : "")}>
                <div className="m-ios-switch-container">
                    <ul>
                        {
                            this.data.options.map((item, index) => {
                                return (
                                    <li data-ref={item.ref} onClick={this.clickHandler.bind(this)} className={(item.active || item.ref == this.state.activeItem ? " active" : "")} key={index}>
    ...
    render() {
        ...
        if (this.data.icon) {
            icon = React.createElement(
                LOL[this.data.icon] || "i",
                { className: "m-form__icon " + (!LOL[this.data.icon] ? "material-" + this.data.icon : "") }
            );
        } else {
            icon = React.createElement(
                ...
                >
                    <div className="m-flex-row m-editinput">
                        <div className="m-editinput-content">
                            ...
                            {
                                this.data.label &&
                                <span>{this.data.label}:&ensp;</span>
                            }
                            <input onChange={e => this.handlerChange(e)} readOnly={this.state.isEdit} type="text" value={this.state.value} />
                        </div>
                        {
                            this.data.disable !== "undefined" && !this.data.disable ?
                                (
                                    this.state.isEdit ?
                                        <span onClick={this.handlerEdit.bind(this)} className="m-action-icon"><LOL.EditOutlined /></span>
                                        :
                                        <span onClick={this.handlerSave.bind(this)} className="m-action-icon"><LOL.SaveOutlined /></span>
                                )
                ...
                <div className="m-flex-row m-editinput">
                    {
                        <div className="m-editinput-content">
                            ...
                            {
                                !this.state.isEdit ?
                                    <div className={"m-editselect m-form__input " + this.state.status + ' ' + (readonly ? 'readonly' : '')}>
                                        <select ref={data.ref} id={data.ref} key={data.ref || ""}
                                    ...
                                    :
                                    <input readOnly={this.state.isEdit} type="text" value={data.content || this.state.content} />
                            }
                        </div>
                    }
                    {
    ...
    render() {
        ...
        return (
            <>
                ...
                >
                    <div style={{ color: "red" }}>* Lưu ý : chỉ được phép đính kèm file hình ảnh, word, excel, pdf</div>
                    <div style={{ color: "red", paddingBottom: "10px" }}>dung lượng mỗi file không được quá 2mb , tổng dung lượng không quá 6mb</div>
                    <Upload
    ...
    render() {
        ...
        return (
            <div className={"m-nonedit-input " + (data.classes || "")}>
                ...
                <span id={this.props.dataSource.id || this.props.dataSource.ref} ref={this.inputRef}>{data.value || ''}</span>
            </div>
        );
    }
}
...
class Mcapcha extends React.Component {
    ...
    render() {
        return (
            <Col flex="auto">
                <img height="32px" id="CapchaImg"
                    src={this.props.dataSource.img ?
    ...
    render() {
        return (
            <Card
                bordered={false}
                {...this.props}
                className={"m-custom-card " + (this.props.background || "") + (this.props.className || "")}
            >
...
class Mbutton extends React.Component {
    ...
    render() {
        return (
            <>
                <Button loading={this.state.loading} {...this.props}>
...
class Msearch extends React.Component {
    ...
    handleChange(e) {
        ...
        if (this.props.config && this.props.config.onLiveSearch) {
            this.props.config.onLiveSearch(val);
        }
    }
    ...
    handleKeyPress(e) {
        ...
        var strchar = (String.fromCharCode(char) || '').replace(checkPrSps, "");
        if (strchar == '') {
            e.preventDefault();
            e.stopPropagation();
        } else {
    ...
    render() {
        var that = this;
        let data = (this.props.config || {}).icon ? this.props.config : this.props.dataSource;
        let icon = "";
        let span = data.span || 24;
        if (data.icon) {
            icon = React.createElement(
        ...
        return (
            ...
            <Col
                ...
                sm={span.sm || span}
                md={span.md || span}
                lg={span.lg || span} style={{ display: (data.isHide === true) ? "none" : "block" }}>
                <div className="m-form__input">
                    ...
                    <Search
                        ...
                        {...((this.props.dataSource || {}).uppercase ? { className: " inputUppercase " + (data.className || '') } : {})}
                        {...((this.props.dataSource || {}).safeString ? { onKeyPress: this.handleKeyPress } : {})}
                        {...this.props.config}
                    ></Search>
                    {icon}
                </div>
...
class Mautocomplete extends React.Component {
    ...
    render() {
        ...
        var options = (data.options || []).map((item, ii) =>
            <Option {...item} key={item.value + '' + ii} value={item.value}>{item.label}</Option>
        );
        if (data.icon) {
            icon = React.createElement(
                LOL[data.icon],
        ...
        return (
            <Col xs={span.xs || span}
                ...
                md={span.md || span}
                lg={span.lg || span}
                xl={span.xl || span} className="m-form__box" style={{ display: (data.isHide === true) ? "none" : "block" }}>
                <div className={"m-form__input " + this.state.status + ' ' + (readonly ? 'readonly' : '')}>
                    <label className={(data.value || this.state.value) ? "m-form__label m-form__label--focus" : "m-form__label"}>{data.label}</label>
                    <AutoComplete style={{ width: "100%" }} ref={data.ref} id={data.ref} key={data.ref || ""}
...
class Mselectsearch extends React.Component {
    ...
    render() {
        ...
        return (
            ...
            >
                <div
                    className={"m-form__input " + this.state.status + ' ' + (readonly ? 'readonly' : '')}
                >
                    ...
                    <Select
                        ...
                        placeholder={placeholder}
                        onChange={this.handleChange.bind(this)}
                        onClear={() => { this.setState({ "value": undefined }) }}
                        onBlur={this.checkBlur.bind(this)}
                        disabled={readonly ? true : false}
                        onFocus={this.checkFocus.bind(this)}
                        ...
                        defaultValue={this.state.value || this.props.dataSource.value || this.state.value || ""}
                ...
                <div className={"m-form__input " + this.state.status + ' ' + (readonly ? 'readonly' : '')}>
                    ...
                    <span className="ant-input-search ant-input-affix-wrapper">
                        ...
                        {(data?.inputType == "search" ? ((this.state.value || data.value || "") ?
                            ...
                            :
                            <span className="ant-input-suffix"><LOL.SearchOutlined className="ant-input-search-icon" onClick={() => { if (typeof (this.props.dataSource || {}).onEnter == "function") (this.props.dataSource || {}).onEnter(this) }} /></span>
                        ) :
                            ''
                        )}
                        {passvisible}
...
class Mdatepicker extends React.Component {
    ...
    checkFocus(e) {
        e.target.parentElement.parentElement.parentElement.getElementsByTagName('label')[0].classList.add('m-form__label--focus');
    }

    checkBlur(e) {
        if (!e.target.value) {
            e.target.parentElement.parentElement.parentElement.getElementsByTagName('label')[0].classList.remove('m-form__label--focus');
        }
    }
                ...
                <div className={"m-form__input " + (this.state.readonly ? 'readonly' : '')}>
                    ...
                    <DatePicker style={{ padding: 0 }} placeholder="" bordered={false} required-text={data.required} required={data.required ? true : false}
                        ...
                        onChange={(date, dateString) => {
                            ...
                            if ((this.props || {}).onChangeValue) {
                                (this.props || {}).onChangeValue(date, dateString, this)
                            }
                        }}
                        // onChange={(date,dateString) => this.handleChange({"date" : date,"value":dateString, "target" : {"id": data.ref,"value":dateString}})} 
                        onBlur={this.checkBlur.bind(this)}
                        onFocus={this.checkFocus.bind(this)} />
                </div>
            </Col>
...
class Mform extends React.Component {
    ...
    render() {
        return (
            <>
                <Row {...this.props} className={this.props.className ? "m-form " + this.props.className : "m-form"}>
...
class Mselect extends React.Component {
    ...
    checkFocus(e) {
        e.target.parentElement.getElementsByTagName('label')[0].classList.add('m-form__label--focus');
    }
        ...
        if (this.props.dataSource) {
            ...
            return (
                <Col
                    ...
                    lg={span.lg || span}
                >
                    <Divider className="m-divider" style={{ margin: "4px 0px 10px" }} orientation={data.align || "center"}>{data.label}</Divider>
                </Col>
            );
        } else {
            return (
...
class MoneFieldInput extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            importContStep: <span>Tiếp theo <LOL.RightCircleOutlined /></span>,
            inputData: {}
        }
    ...
    nextStep() {
        ...
        if (this.state.currentIndex == this.state.maxLength - 2) {
            this.setState({
                ...
                importContStep: <span>Hoàn tất <LOL.UpCircleOutlined /></span>
            })
            return;
        }
        this.setState({
    ...
    renderInput(data) {
        return (
            <>
                ...
                <Input autoFocus onChange={(e) => this.handleChange(e)}
                    value={this.state.inputData[data.ref]}
                    key={data.ref} id={data.ref} />
            </>
        );
    }
            ...
            <>
                ...
                >
                    {data.option.map((item, ii) => {
                        return <Option key={ii} value={item.value}>{item.label}</Option>
                    })}
                </Select>
            </>
        );
                ...
                <Row justify="center">
                    ...
                    >
                        <div className="m-1field-input" onKeyPress={this.keypressHandler}>
                            <div
                                className={"m-addon-before " + (this.state.currentIndex == 0 ? "m-disable" : "")}
                                onClick={() => this.prevStep()}
                            >
                                <LOL.LeftCircleOutlined /> Quay lại</div>
                            {this.state.dataList[this.state.currentIndex]}
                            <div
                                className={"m-addon-after"}
